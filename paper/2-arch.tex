...
As the selected data word must be configured per microinstruction in microcode before computer execution. Each data word is assigned a purpose. The first one storing the instruction and the second storing the operand.    
...
For certain features, such as a call stack, it is nonetheless useful to allow for absolute addressing. Additionally, first 8-bit of the PC and MAR shall be over writeable for absolute access.
